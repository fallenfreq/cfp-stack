	...
	<VaCard class="p-6">
		...
		<template v-if="emailStatus === 'idle' || emailStatus === 'sending'">
			...
			<p
				...
				style="background: color-mix(in srgb, var(--va-warning) 15%, transparent); color: var(--va-warning)"
			>
				This change takes effect immediately. If you cannot access the verification email sent to
		...
		<template v-else-if="emailStatus === 'code' || emailStatus === 'verifying'">
			<p class="text-sm mb-4" style="opacity: 0.8">
				A verification code has been sent to <strong>{{ pendingEmail }}</strong>. Your current
				email remains active until you verify the new one.
			</p>
			...
			<div class="flex items-center gap-3 mt-3">
				<VaButton preset="plain" size="small" @click="resendEmailCode">Resend code</VaButton>
		...
		<div v-else-if="emailStatus === 'done'" class="flex items-center gap-4">
			<p class="text-sm" style="color: var(--va-success)">
				Email updated to {{ pendingEmail }}. Sign out and back in to see the change reflected here.
			</p>
			<VaButton preset="secondary" size="small" @click="cancelEmailChange">Change again</VaButton>
		</div>
	</VaCard>
</template>
